...
public interface DictionaryCategoryMapper {
    ...
    @Select("SELECT * FROM dictionary_category WHERE id=#{id}")
    public DictionaryCategory getById(UUID id);

    @Insert("INSERT INTO dictionary_category(id, name) VALUES(#{id}, #{name})")
    void insert(DictionaryCategory dictionaryCategory);

    @Update("UPDATE dictionary_category SET name=#{name} WHERE id=#{id}")
    void update(DictionaryCategory dictionaryCategory);

    @Delete("DELETE FROM dictionary_category WHERE id=#{id}")
    void delete(UUID id);
}
